...
\begin{tabular}{|c|c|c|}
\hline
\textbf{eBPF programs} & \textbf{\#Instructions} & \textbf{\#C lines}\\  \hline
Return                & 4                         & 8\\ \hline
Wire Fast             & 5                         & 8\\ \hline
%wire                  & 7                       &\\ \hline
...
%l4lb                  & 155                     & \\ \hline
NAT                   & 182                      & 229\\ \hline
Chacha 8  (Loop Unroll off) &     575            & 356\\ \hline
Chacha 20 (Loop Unroll off) &     575          & 356\\ \hline
Chacha 8  (Loop Unroll on)  &     1,543        & 356\\ \hline
Chacha 20 (Loop Unroll on)  &     3,965        & 356\\ \hline
\end{tabular}}
\end{table}
...
\textcolor{red}{The first application forwards frames from one interface to another. There is no learning involved, the output ports are hard-coded. Using the iperf tool, UDP connections are created between terminals A and B. Figure %~\ref{fig:07result} 
shows the UDP goodput per packet size. This experiment evaluates what is the maximum obtained goodput.}

The second example is an IPv4 router. When eBPFlow receives a frame, it checks if the frame is an Ethernet frame and if it contains an IPv4 packet. It extracts the IPv4 destination and searches the TCAM for the output port. The available actions are to forward the packet or drop it. Figure %~\ref{fig:IPv4routerresult} 
